...
const FIELDS: Array<{
  ...
  {
    key: 'lowDetailMaxNodes', label: '低细节档最多挂载节点数', step: 500,
    describe: '缩放很小、卡片已退化为色块时用的独立预算。'
      + '「缩到 10% 时中间一块有内容、外面整片消失」就是这一项在起作用。',
    cost: '低缩放下每个节点只是个色块，成本比满细节低一个量级，'
      + '所以这一项通常可以远高于上面那项；弱机器则可以压得更低。',
  },
  {
