...
4. 点击 `点击生成` / `重试` 只触发 `onNodeAction`，不改变选中集——两侧各写一条测试
   ...
   ⏸ **本条正式延期到 `D0-min`**（2026-08-04 裁定）。原因：本规格 §5 依赖 `RenderContext.callbacks`，而该字段在编排上被排到了波次 3 的 D0——**是任务排序时的依赖倒置**，由 Codex 在 C1-dom 开工核对时发现并停下上报。
   
   波次 2 只做**视觉 + 把内部按钮按「不参与选中/拖拽/双击」的语义标记出来**；按钮的点击行为与本条验收统一在 `D0-min` 补齐。
   
   延期而不是就地修，是因为当时 Kimi 正在飞 C1-leafer——`callbacks` 会同时牵动 `core` 与两侧渲染器，**在它跑到一半时改 `core` 会打断它的运行**。
