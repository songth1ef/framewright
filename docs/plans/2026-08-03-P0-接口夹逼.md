...
3. **`pnpm verify` 是提交门禁**：`typecheck && test && e2e` 三条串起来，每次 commit 前跑一遍。**不允许用 `--no-verify` 之类的方式绕过**（见 `AGENTS.md` §3 根因优先）。
...
以下为全局约束，**每个任务的要求都隐含包含本节**。来源为 `AGENTS.md` 与 `docs/*.md`，值逐字照抄。
