...
>
> **⚠️ 订正声明（2026-08-03）**：本作答基于旧版规格（「纯滚轮=缩放 + 触控板双指=平移」）。
> 作答完成后规格于 `f95af3b` 改为 **滚轮=平移 / `Ctrl`+滚轮=缩放**（Figma 方案，用户拍板）。
> 受影响的条目在文末「附：按新规格订正」统一订正，正文原样保留作对照。
> 有意思的是：旧组合的技术不成立（无法区分滚轮与双指）正是本作答 A-Q4② / Q8④
> 独立指出的风险，新规格的技术前提论证与此一致。
>
...
## 附：按新规格订正（`f95af3b`：滚轮=平移 / `Ctrl`+滚轮=缩放）

以下订正覆盖正文中基于旧规格（纯滚轮=缩放）的条目。未列出的条目（C–J、Q6、Q7）
不受视口方案变更影响，原样有效。

### 订正 A（平移）

- **滚轮从「不相关」变成平移的主触发**：垂直滚轮=垂直平移、`Shift`+滚轮=水平平移、
  触控板双指=平移，三者是同一个 `wheel` 事件、同一条规则。监听方式不变：
  原生 `wheel`（`passive: false`）+ 关闭 Leafer 内建 wheel 消费。
- **A-Q4② / Q8④ 的启发式难题被新规格根除**：不再需要区分鼠标滚轮与触控板双指，
  零判别、零启发式。原答案中「启发式必须放 core 共用」一条作废——这是新规格
  最大的技术红利。
- `panBy(viewport, deltaScreenX, deltaScreenY)` 的需求不变，且用量更大了
  （滚轮每次触发都走它）。

### 订正 B（缩放）

- **触发改为 `Ctrl`/`Cmd`+滚轮**（鼠标）与**触控板捏合**（浏览器合成为
  `ctrlKey === true` 的 wheel），天然同一条规则。监听方式不变。
- **「×1.1 步进 vs 连续 delta 归一化」的问题依然存在**：捏合合成的 ctrl+wheel
  是小额连续 delta，鼠标 Ctrl+滚轮是一格一格的大 delta——「格数」归一化常数
  仍需 core 统一，否则鼠标与触控板的缩放速度手感不一致。
  （变化的是：不再涉及 DOM/Leafer 两侧分歧——事件都在原生层，归一化放 core
  后两侧天然一致。）

### 订正 Q8（事件冲突）

- 原第 4 条（双指 vs 滚轮的无修饰键区分）**冲突消解**，删除。
- 其余各条（内建手势泄漏、wheel 双消费、DOWN 三分支、中键/空格、pointercancel）
  不受影响，仍然成立。
