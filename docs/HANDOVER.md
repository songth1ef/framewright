...
- **探针首屏证据依赖外网取图** —— 800% 档的证据节点是图片，等 picsum 下载；
  实测长尾偶尔超过 20 秒，导致 2 个档位 partial（`load` 记录证明机器是闲的，不是环境问题）。
  要根治需让探针本地供图、或在首屏计时前预热该图；两种改法都会改变现有数字口径，
  属独立一块，**没做就不假装有**（详见 `architecture.md` §8.9.3）
...
- **Leafer 探针在 800% 档必然失败（根因已查清，修复未成）**：
  ...
  **两个假设都已被证伪，不要重复走**：
  1. ~~「ai-image 图片区没下完」~~ —— 800% 下唯一与视口相交的节点是
     `scale-node-0`，实测它是 **`img` 类型不是 `ai-image`**（没有生成单元 chrome）
  2. ~~「等的帧数不够」~~ —— 把等待从 10 帧放到 **600 帧（约 10 秒，覆盖 picsum
     的 5.5~6.6 秒首字节）仍然失败**

  ✅ **错误透传已补（2026-08-07）**：三个 runner 的失败样本现在带 `errorDetail`
  字段（完整消息，含 `page.evaluate` 的真实异常与调用栈），不再只留第一行。
  用它重跑 800% 档立刻看到：
  `page.evaluate: Error: 首屏 10 帧后仍无 scale-node-0 像素` + 完整调用栈。

  **接手时从 `errorDetail` 读起，别再从假设开始。**

  ⚠️ **投入产出提示**：此项只影响 800% 档的探针取证，不影响产品，也不影响其余
  24 个组合的数据。三方报告里那三格标的是明确的 `timeout`（测不到），
  不是编造的数字。修它的优先级不高。
...
### 7.3.2 2026-08-08 新增的四条（都来自同一夜的连锁）

- **用坏掉的仪表做的证伪，不构成证伪**：Leafer 800% 三个 timeout 此前有两条「已证伪」
  的假设（「等的帧数不够，600 帧仍失败」「不是图片问题，该节点是 `img` 不是 `ai-image`」）。
  两条都是用**采样点打在屏幕外**的探针测的，结论一律作废；第二条的推理本身也不成立
  —— `img` 就是图片。→ 推翻假设前先确认仪表是好的
- **诊断信息默认保留，不要为了输出简洁而截断**：为了让单档失败不带走整轮，我给失败样本
  加了 `error.message.split('\n')[0]`，把子进程 stderr 里的真实错误砍掉了，
  代价是随后两轮、四五次盲猜。控制台可以只显示一行，**落盘的数据必须完整**。
  补上 `errorDetail` 后一次定位
- **「能部分交付」只做一半等于没做**：`benchmark.mjs` 早写着「跑一小时的东西必须能部分
  交付」，但只用 try/catch 防了「某渲染器抛错」，没防「进程整个死掉」—— 全量矩阵跑到
  38/81 被杀，一小时数据一条没留。改成逐档 flush 后**仍是 0 存活**，因为产物写在
  `os.tmpdir()` 里、退出时被 `finally` 删了。**「逐档落盘」和「落在会被删掉的地方」
  放一起，等于没做。** 移到 `benchmarks/results/run-<时间戳>/` 后实测 `kill -9` 下 9 档全活
- **涉及「当前是什么」的判断，以代码为准，不以文档为准**：`architecture.md` 有句
  「默认渲染器目前是 LeaferJS」早已过期（实际 `DEFAULT_SETTINGS.renderer === 'dom'`），
  本方据它向用户报告了一条**根本不存在的待决策**。过期结论会被当成事实二次传播
